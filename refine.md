...
> 路径基准：`packages/killstata/`（另有仓库根目录项单独标注）。

---

## ✅ 状态：全部 19 项已完成（2026-07-12）

本文档已从「待办清单」变为「已完成记录」，保留供追溯。当前进度见 [PROGRESS.md](./PROGRESS.md)。

| 类别 | 项目 | 状态 |
|---|---|---|
| A 类高危 | B1、B4、B5 | ✅ 已修（面板 FE 已迁移 linearmodels，黄金数据集验证） |
| B 类静默失效 | B2、B3、B7、B8、B9、B10、B11 | ✅ 已修 |
| C 类死代码/遗留 | C1、C2、C3、C4、C5、C6、C7、C8、C9 | ✅ 已清 |

**执行中新发现并修复的、本文档之外的 5 个问题**（静态审查看不出，靠真跑测试暴露）：
1. 主脚本 6 个函数只存在于从未被调用的死代码里 → **OLS/DID/IV/PSM/RDD 此前全部 NameError 崩溃**
2. `nested_pvalue` 键名对不上第三方库实际返回的 `lm_pvalue` → HC1 自动升级从未触发
3. HC1 升级路径丢失 pandas 索引 → 崩溃被 `except: pass` 静默吞掉
4. `build_model_qa` 误把重复面板键判为 blocking_errors → 自动降级逻辑永远走不到
5. `heterogeneity-runner.ts` 还有一处裸 `pd.read_csv`（B1/B3 的漏网之鱼）

**两处对原方案的刻意偏离**（原方案会造成问题，实测后修正）：
- C4 说 `ls.ts` 是孤儿工具可删 → 实测 `prompt.ts:1395` **直接调用**它，已恢复保留
- C6 说 `ModelsDev.get()` 返回 `{}` → 实测会让 `killstata config` 向导变空列表、用户无法配 key，
  改为返回内置二元目录（deepseek + custom）
