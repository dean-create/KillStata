...
- 2026-07-17：已完成“真实论文数据 + DeepSeek 工具调用回放验收基座”设计并写入 PLAN：统一 AcceptanceCase、方法专用数据注册表、生产 Harness trace、独立数值 oracle、真实模型 3 次稳定性回放、失败参数门禁和准入报告；首个 pilot 定为 `panel_fe_regression + did.xlsx`。本轮补上 PSM 面板硬门禁：`psm_matching/psm_ipw` 均要求 `analysisUnitVar + preTreatmentAggregation`，Python 会拒绝当前 stage 中重复分析单位；真实 city×year 复现已转绿。`psm_ipw` 当前固定 Hájek ATE，Card 仅为 B 级 smoke；LaLonde/NSW A 级对标仍待专用夹具完成。
