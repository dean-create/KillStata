...
description: What's new in Killstata 1.0.
---

Killstata 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same killstata server.
...
version. However some older versions of Killstata always grab latest.

To upgrade manually, run

```bash
$ killstata upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ killstata upgrade 0.15.31
```
