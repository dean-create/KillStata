...
You can add external tools to Killstata using the _Model Context Protocol_, or MCP. Killstata supports both local and remote servers.
...
You can define MCP servers in your [Killstata Config](https://killstata.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="killstata.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/killstata` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="killstata.jsonc" {15}
{
  "$schema": "https://killstata.io/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Killstata automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Killstata will:
...
If the server requires authentication, Killstata will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `killstata mcp auth <server-name>`.
...
```json title="killstata.json" {7-11}
{
  "$schema": "https://killstata.io/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
killstata mcp auth my-oauth-server
```
...
```bash
killstata mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Killstata will store the tokens securely in `~/.local/share/killstata/mcp-auth.json`.
...
killstata mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Killstata, alongside built-in tools. So you can manage them through the Killstata config like any other tool.
...
```json title="killstata.json" {11, 14-18}
{
  "$schema": "https://killstata.io/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
killstata mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Killstata to your Sentry account.
