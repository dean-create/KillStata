...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Killstata.
...
Here are several models that work well with Killstata, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Killstata Zen](/docs/zen), you would use `killstata/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="killstata.jsonc" {7-12,19-24}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "openai": {
...
```jsonc title="killstata.jsonc" {6-21}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "killstata": {
...
Many models support multiple variants with different configurations. Killstata ships with built-in default variants for popular providers.

### Built-in variants

Killstata ships with default variants for many providers:
...
```jsonc title="killstata.jsonc" {7-18}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "openai": {
...
When Killstata starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Killstata config.
