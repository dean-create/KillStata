---
title: Server
description: Interact with killstata server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `killstata serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an killstata client can use.
...
```bash
killstata serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
killstata serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `KILLSTATA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `killstata`, or set `KILLSTATA_SERVER_USERNAME` to override it. This applies to both `killstata serve` and `killstata web`.

```bash
KILLSTATA_SERVER_PASSWORD=your-password killstata serve
```
...
When you run `killstata` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the killstata server to interact with killstata programmatically.
:::

This architecture lets killstata support multiple clients and allows you to interact with killstata programmatically.

You can run `killstata serve` to start a standalone server. If you have the
killstata TUI running, `killstata serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Killstata [IDE](/docs/ide) plugins.
...
The killstata server exposes the following APIs.
