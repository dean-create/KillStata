---
title: SDK
description: Type-safe JS client for killstata server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The killstata JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control killstata programmatically.
...
Create an instance of killstata:

```javascript
import { createKillstata } from "@killstata/sdk"

const { client } = await createKillstata()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `killstata.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${killstata.server.url}`)
...
If you already have a running instance of killstata, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@killstata/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*killstata" },
})
