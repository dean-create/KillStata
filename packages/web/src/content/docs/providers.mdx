---
title: Providers
description: Using any LLM provider in Killstata.
---

import config from "../../../config.mjs"
export const console = config.console

Killstata uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Killstata config.
...
in `~/.local/share/killstata/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Killstata
config.
...
Killstata Zen is a list of models provided by the Killstata team that have been
tested and verified to work well with Killstata. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Killstata Zen.
:::

1. Run the `/connect` command in the TUI, select killstata, and head to [killstata.ai/auth](https://killstata.ai/auth).
...
It works like any other provider in Killstata and is completely optional to use.
...
To use Amazon Bedrock with Killstata:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running killstata:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY killstata

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile killstata

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX killstata
   ```
   ...
   For project-specific or persistent configuration, use `killstata.json`:

   ```json title="killstata.json"
   {
     "$schema": "https://killstata.io/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
We've received reports of some users having their subscriptions blocked while using it with Killstata.
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for killstata to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX killstata
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX killstata
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your killstata config.
...
to store token in killstata auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Killstata uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Killstata
to only use your own GitLab-hosted instance, add the following to your
`killstata.json` file. It is also recommended to disable session sharing.
...
More documentation on [killstata-gitlab-auth](https://www.npmjs.com/package/@gitlab/killstata-gitlab-auth) homepage.

##### Configuration

Customize through `killstata.json`:
...
{
  "$schema": "https://killstata.io/config.json",
  "plugin": ["@gitlab/killstata-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with killstata:
...
To use Google Vertex AI with Killstata:
...
2. Set the required environment variables:
   ...
   Set them while running killstata.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id killstata
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through killstata, you can always configure it yourself.
...
```jsonc title="~/.config/killstata/killstata.jsonc"
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [killstata-helicone-session](https://github.com/H2Shami/killstata-helicone-session) plugin to automatically log each Killstata conversation as a session in Helicone.

```bash
npm install -g killstata-helicone-session
```
...
```json title="killstata.json"
{
  "plugin": ["killstata-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Killstata conversation listed as a separate session.
...
You can configure killstata to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="killstata.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure killstata to use local models through LM Studio.

```json title="killstata.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "lmstudio": {
...
You can configure killstata to use local models through Ollama.

```json title="killstata.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Killstata:
...
3. Copy the API key for use in Killstata.
...
6. **Important**: Before using cloud models in Killstata, you must pull the model information locally:
...
Killstata Zen is a list of tested and verified models provided by the Killstata team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Killstata Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Killstata Zen**.

   ```txt
   /connect
   ```

3. Enter your Killstata API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your killstata config.
...
5. You can also customize them through your killstata config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' killstata
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group killstata
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Killstata:
...
You can also customize models through your killstata config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with killstata. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in killstata.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `killstata.json` file in your project directory:

   ```json title="killstata.json" ""myprovider"" {5-15}
   {
     "$schema": "https://killstata.io/config.json",
     "provider": {
       "myprovider": {
...
```json title="killstata.json" {9,11,17-20}
{
  "$schema": "https://killstata.io/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Killstata to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `killstata auth list` to see if the credentials
...
2. For custom providers, check the killstata config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your killstata config.
