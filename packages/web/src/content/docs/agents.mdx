...
There are two types of agents in Killstata; primary agents and subagents.
...
Killstata comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Killstata comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Killstata comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `killstata.json` config file:
...
- Global: `~/.config/killstata/agents/`
- Per-project: `.killstata/agents/`

```markdown title="~/.config/killstata/agents/review.md"
---
...
If no temperature is specified, Killstata uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Killstata config and the project specific config.
...
The model ID in your Killstata config uses the format `provider/model-id`. For example, if you're using [Killstata Zen](/docs/zen), you would use `killstata/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="killstata.json" {3-6,9-12}
{
  "$schema": "https://killstata.io/config.json",
  "tools": {
    "write": true,
...
```json title="killstata.json" {3-5,8-10}
{
  "$schema": "https://killstata.io/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `killstata models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/killstata).
:::
...
```markdown title="~/.config/killstata/agents/docs-writer.md"
---
...
```markdown title="~/.config/killstata/agents/security-auditor.md"
---
