...
With Killstata you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Killstata uses our own `killstata` theme.
...
Killstata comes with several built-in themes.
...
- Want Killstata to match their terminal's appearance
...
Killstata supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/killstata/themes/*.json` or `$XDG_CONFIG_HOME/killstata/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.killstata/themes/*.json`
4. **Current working directory** - Defined in `./.killstata/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/killstata/themes
vim ~/.config/killstata/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .killstata/themes
vim .killstata/themes/my-theme.json
```
...
{
  "$schema": "https://killstata.io/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
