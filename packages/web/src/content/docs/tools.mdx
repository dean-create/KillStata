...
Tools allow the LLM to perform actions in your codebase. Killstata comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Killstata.
...
:::note
This tool is only available when `KILLSTATA_EXPERIMENTAL_LSP_TOOL=true` (or `KILLSTATA_EXPERIMENTAL=true`).
:::
