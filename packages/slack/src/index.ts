...
import { createKillstata, type ToolPart } from "@killstata/sdk"
...
console.log("🚀 Starting killstata server...")
const killstata = await createKillstata({
  port: 0,
})
console.log("✅ Killstata server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await killstata.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new killstata session...")
    const { client, server } = killstata
    ...
    console.log("✅ Created killstata session:", createResult.data.id)
  ...
  console.log("📝 Sending to killstata:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Killstata response:", JSON.stringify(result, null, 2))
