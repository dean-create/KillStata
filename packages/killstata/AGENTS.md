...
- **API Client**: The TypeScript TUI (built with SolidJS + OpenTUI) communicates with the Killstata server using `@killstata/sdk`. When adding/modifying server endpoints in `packages/killstata/src/server/server.ts`, run `./script/generate.ts` to regenerate the SDK and related files.
