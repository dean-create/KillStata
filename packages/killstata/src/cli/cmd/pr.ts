...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run killstata",
  builder: (yargs) =>
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found Killstata session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`killstata import ${sessionUrl}`.nothrow()
