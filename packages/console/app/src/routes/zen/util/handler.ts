...
import { and, Database, eq, isNull, lt, or, sql } from "@killstata/console-core/drizzle/index.js"
import { KeyTable } from "@killstata/console-core/schema/key.sql.js"
import { BillingTable, SubscriptionTable, UsageTable } from "@killstata/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@killstata/console-core/util/price.js"
import { getWeekBounds } from "@killstata/console-core/util/date.js"
...
import { WorkspaceTable } from "@killstata/console-core/schema/workspace.sql.js"
import { ZenData } from "@killstata/console-core/model.js"
import { Black, BlackData } from "@killstata/console-core/black.js"
import { UserTable } from "@killstata/console-core/schema/user.sql.js"
import { ModelTable } from "@killstata/console-core/schema/model.sql.js"
import { ProviderTable } from "@killstata/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // killstata bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-killstata-session") ?? ""
    const requestId = input.request.headers.get("x-killstata-request") ?? ""
    const projectId = input.request.headers.get("x-killstata-project") ?? ""
    const ocClient = input.request.headers.get("x-killstata-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-killstata-request")
          headers.delete("x-killstata-session")
          headers.delete("x-killstata-project")
          headers.delete("x-killstata-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo) {
    ...
    if (!billing.paymentMethodID)
      throw new CreditsError(
        `No payment method. Add a payment method here: https://killstata.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    if (billing.balance <= 0)
      throw new CreditsError(
        `Insufficient balance. Manage your billing here: https://killstata.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new MonthlyLimitError(
        `Your workspace has reached its monthly spending limit of $${billing.monthlyLimit}. Manage your limits here: https://killstata.ai/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new UserLimitError(
        `You have reached your monthly spending limit of $${authInfo.user.monthlyLimit}. Manage your limits here: https://killstata.ai/workspace/${authInfo.workspaceID}/members`,
      )
  }
