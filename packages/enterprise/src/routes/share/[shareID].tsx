import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@killstata/sdk/v2"
import { SessionTurn } from "@killstata/ui/session-turn"
import { SessionReview } from "@killstata/ui/session-review"
import { DataProvider } from "@killstata/ui/context"
import { DiffComponentProvider } from "@killstata/ui/context/diff"
import { CodeComponentProvider } from "@killstata/ui/context/code"
import { WorkerPoolProvider } from "@killstata/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@killstata/ui/logo"
import { IconButton } from "@killstata/ui/icon-button"
import { ProviderIcon } from "@killstata/ui/provider-icon"
import { createDefaultOptions } from "@killstata/ui/pierre"
...
import { Diff as SSRDiff } from "@killstata/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@killstata/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@killstata/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@killstata/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@killstata/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/killstata-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Killstata</Title>
              </Show>
              <Meta name="description" content="killstata - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/killstata"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://killstata.ai/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
