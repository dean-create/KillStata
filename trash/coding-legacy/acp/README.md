...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for killstata.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal killstata sessions
...
killstata acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via killstata's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | killstata acp
```
...
### Mapping to Killstata

ACP sessions map cleanly to killstata's internal session model:
